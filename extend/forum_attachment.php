...
if(empty($_GET['nothumb']) && $attach['isimage'] && $attach['thumb']) {
	...
	if($attach['remote']) {
		...
		// getlocalfile($_G['setting']['attachdir'].'/forum/'.getimgthumbname($attach['attachment']), $attach);
		getlocalfile($_G['setting']['attachdir'].'/'.getimgthumbname($attach['attachment']), $attach);
	}
	exit();
}
...
$filename = $_G['cache']['plugin']['qiniu']['url'].$attach['attachment'];
$attachInfo = maile\qiniu::getInfo($attach['attachment']);

if(!$attach['remote'] && !$attachInfo) {				//附件无法读取
...
$filesize = !$attach['remote'] ? $attachInfo['fsize'] : $attach['filesize'];
...
$attach['remote'] ? getremotefile($attach['attachment']) : getlocalfile($filename, $attach);
...
function getlocalfile($url, $attach){
	global $_G;
	if($_G['cache']['plugin']['qiniu']['protect']){
		...
		$url = maile\qiniu::getDownURL($url, 5);
	}
