...
if(empty($_G['uid']) && $_GET['operation'] != 'upload') {
	exit();
} else {
	if($_G['uid']) {
...
// 引入上传类
require_once DISCUZ_ROOT . 'source/plugin/qiniu/extend/forum_upload.php';
...
if($_GET['operation'] == 'upload') {
	...
	if(!$errorcode) {
		...
		if($image['uid'] == $_G['uid']) {
			...
			@unlink($_G['setting']['attachdir'].'/'.$image['attachment']);
			@unlink($_G['setting']['attachdir'].'/'.getimgthumbname($image['attachment']));
			$attach['attachid'] = $aid;
		} else {
